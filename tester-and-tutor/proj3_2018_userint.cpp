...
void userint::printSchedule(proj3_2018_student stu, proj3_2018_schedule sched) { 
	...
	for (int i = 0; i < sched.getNumCourses(); i++) {
		...
		proj3_2018_course crs = cat.getCourse(crsNdx);     // get a copy of the Course data from the cat
...
void userint::addCourse(proj3_2018_student stu, proj3_2018_schedule & sched) { 
...
void userint::dropCourse(proj3_2018_student stu, proj3_2018_schedule & sched) { 
...
void userint::registerStudent() {
	...
	proj3_2018_student stu = stuList.search(stuId);
	if (stu.getId() == "") {
	...
	proj3_2018_schedule sched = scList.getSchedule(stuId);
